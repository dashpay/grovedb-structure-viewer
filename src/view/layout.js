...
export const GRID = { width: 232, maxWidth: 420, height: 104, gap: 16, pad: 28, narrowPad: 16 };
...
export function gridLayout(ids, stageWidth) {
  ...
  const columns = Math.max(1, Math.min(ids.length || 1, Math.floor((stageWidth - GRID.pad * 2 + gap) / (GRID.width + gap))));
  // A single column takes the stage's width, so a phone is not mostly margin.
  // Columns are counted with one padding so their number only grows with the width.
  const pad = columns === 1 ? GRID.narrowPad : GRID.pad;
  const width = columns === 1 ? Math.max(160, Math.min(GRID.maxWidth, stageWidth - pad * 2)) : GRID.width;
